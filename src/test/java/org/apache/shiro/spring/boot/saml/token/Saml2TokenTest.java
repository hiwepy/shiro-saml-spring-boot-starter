 ...
 * @since 1.0.0
 */
@DisplayName("Saml2Token Tests")
class Saml2TokenTest {

    @Test
    @DisplayName("Constructor with host, SAMLRequest, and rememberMe")
    void testConstructor() {
        Saml2Token token = new Saml2Token("localhost", "saml2-request", true);
        assertThat(token).isNotNull();
        assertThat(token.getHost()).isEqualTo("localhost");
        assertThat(token.getSAMLRequest()).isEqualTo("saml2-request");
    ...
    void testGetPrincipal() {
        Saml2Token token = new Saml2Token("localhost", "saml2-request", false);
        assertThat(token.getPrincipal()).isEqualTo("saml2-request");
    }
    ...
    void testGetCredentials() {
        Saml2Token token = new Saml2Token("localhost", "saml2-request", false);
        assertThat(token.getCredentials()).isEqualTo("saml2-request");
    }
}
